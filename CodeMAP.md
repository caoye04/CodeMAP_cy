...
实现思路：Tree-sitter 是 GitHub 开发的增量解析器生成框架，目前支持 130+ 种语言，Python 有现成 binding：
...
Python       → ast 模块（精确，零依赖） C/C++        → libclang 或 tree-sitter（精确签名和类型） 其他支持语言  → tree-sitter-languages（统一覆盖 130+ 语言） 极小众语言    → ctags 兜底，只拿名称和行号，io 留空 真正无解的    → 记录 language=Unknown，跳过函数提取，不调 LLM
